    ...
    },
    {
      ...
      render: row => (
        <>
          ...
          <IconButton size='small' color={'secondary'} onClick={() => handlePartCode(row.id)}>
  ...
  const handleSave = () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const saveBrand = async () => {
        try {
          const response = isEdit ? await api.put('/SpareParts/Update', params) : await api.post('/SpareParts', params)
      ...
      saveBrand().then(() => {
        if (is_successful) {
          ...
          setTimeout(
            () => toast.success(isEdit ? t('Spare Part updated successfully!') : t('Spare Part created successfully!')),
            800
          )
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={handleClose}>
        {isEdit ? (
          <DialogTitle>{t('editSparePart')}</DialogTitle>
        ) : (
          <DialogTitle>{t('createNewSparePart')}</DialogTitle>
        )}
      ...
      <Dialog open={openPartCodeModal} onClose={handlePartCodeClose} fullWidth>
        ...
        <form noValidate autoComplete='off' onSubmit={handlePartCodesSave} className='flex flex-col gap-5'>
          <DialogContent>
            <TextField
              autoFocus
              margin='dense'
